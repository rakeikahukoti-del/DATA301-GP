...
The `pct_pos_total` variable is the percentage of positive reviews for a game. We set a minimum size of 100 games per genre before comparing review scores, since without a cutoff a single game in a small tag could push its median to 100% or 0% and rank it above established genres like Action or RPG. This cutoff removes every software tag and keeps the 10 significant game genres shown in Figure 4. Casual has the highest median at 83%, and Massively Multiplayer has the lowest at 67%. The remaining genres sit close together between these two, and their boxes overlap heavily, showing that genre alone does not explain review positivity well.

![](images/figure4.svg){fig-align="center" width="450"}
...
The `price` variable is the base price of a game in US dollars, and `required_age` is the minimum age rating Steam sets for a game. Figure 5 shows the price spread for each genre. Sports and RPG have the highest median price at \$7.99, while Massively Multiplayer sits close to \$0 since most of these games are free to play. The remaining genres fall between these two, generally in the \$3-7 range. We also checked the share of games with an age restriction in each genre. Figure 6 shows Action has the highest share at 3.9%, compared to 0.5% for Casual. This pattern lines up with the review score chart in reverse: genres with a higher price and a higher age-restriction share tend to have a lower median `pct_pos_total`. This supports the plan to control for both price and age when we study the link between genre and review positivity.
...
`positive` is a variable in the dataset that gives the total number of positive reviews a game has received on Steam. It uses a raw count rather than a percentage used in `pct_pos_total`. It reflects both how many people reviewed a game and how many of those reviews were positive. Figure 9 shows the price plotted against `positive` and `average_playtime_forever`. Most games are priced under \$25 and it gets increasingly sparse beyond \$75 with a few outliers at \$200. We plotted both charts on a log scale because positive spans from single digit counts to over 10 million, making the spread easier to see. `average_playtime_forever` shows a similarly wide range from near 0 up to 10\^6. In both panels, the highest values of `positive` and `average_playtime_forever` are concentrated among games priced below \$25. Games priced above \$75 show a narrower range of both variables. There is also a visible band of points at the bottom of the `average_playtime_forever` panel, consistent with the large share of games with 0 average playtime discussed earlier.

![](images/figure3-01.svg){fig-align="center" width="550"}

**Figure 9:** Price vs Positive reviews (raw count) and Average Playtime
...
![](images/figure4-02.svg){fig-align="center" width="400"}
